...
.. |number_of_audio_algorithms| replace:: 14
.. |number_of_image_attacks| replace:: 30+
.. |number_of_audio_attacks| replace:: 30
...
* :ref:`Datasets <datasets-link>`: `DiffusionDB`_, `MS COCO`_, `LibriSpeech`_, `AudioSet`_,  `LibriTTS`_, `FMA`_, `VCTK`_, `CommonVoice`_, `AISHELL-1`_, `Golos`_ supported
