...
from wibench.utils import overlay_difference
...
class RobustWideWrapper(BaseAlgorithmWrapper):
    ...
    def __init__(self, params: Dict[str, Any] = {}) -> None:
        ...
        self.normalize = transforms.Compose([transforms.Normalize([0.5], [0.5])])
        self.denormalize = transforms.Compose([transforms.Normalize(mean=[-1.0], std=[2.0])])
    ...
    def embed(self, image: TorchImg, watermark_data: TorchBitWatermarkData) -> TorchImg:
        ...
        """
        resized_image = self.transform(image).to(self.device)
        normalized_image = self.normalize(resized_image).unsqueeze(0).clamp(-1, 1)
        encoded_image = self.model.encoder(normalized_image, watermark_data.watermark.float().to(self.device))
        marked_image = overlay_difference(image.to(self.device), resized_image,  self.denormalize(encoded_image.squeeze(0)).clamp(0, 1))
        return marked_image.detach().cpu()
    ...
    def extract(self, image: TorchImg, watermark_data: TorchBitWatermarkData) -> torch.Tensor:
        ...
        resized_image = self.transform(image).to(self.device)
        normalized_image = self.normalize(resized_image).unsqueeze(0).clamp(-1, 1)
        extracted_bits = self.model.decoder(normalized_image)
