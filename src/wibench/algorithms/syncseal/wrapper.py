...
class SyncSeal(BaseAlgorithmWrapper):
    ...
    def _bulid_from_config(self, checkpoint_path: str) -> nn.Module:
        with ModuleImporter("syncseal", self.module_path):
            ...
            logger.info(f'embedder: {sum(p.numel() for p in embedder.parameters() if p.requires_grad) / 1e6:.1f}M parameters')
            ...
            logger.info(f'extractor: {sum(p.numel() for p in extractor.parameters() if p.requires_grad) / 1e6:.1f}M parameters')
